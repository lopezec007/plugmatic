...
public static class ReadCommand
{
    public static Command Build()
    {
        ...
        cmd.SetAction(async (pr, ct) =>
            await RunAsync(Common.Resolve(pr.GetValue(radio)), pr.GetValue(port), ct));
        return cmd;
    }

    public static async Task<int> RunAsync(IRadioDefinition radio, string? port, CancellationToken ct, string? tag = null)
    {
        var runs = new RunManager();
        var run = runs.CreateRun(radio.Model, "read");
        if (tag is not null) run.Tags.Add(tag);
        Console.WriteLine($"Run: {run.Directory}");
        var outcome = RunOutcome.Failed;
        try
        {
            await using var session = await RadioSession.OpenAsync(radio, port, run, ct);
            ...
            var ir = radio.Codec.Decode(image);
...
{
    public static Option<string> RadioOption() => new("--radio")
    {
        Description = $"Radio model ({RadioRegistry.SupportedList()})",
        DefaultValueFactory = _ => "dm32uv",
    };

    public static IRadioDefinition Resolve(string? model)
    {
        if (!RadioRegistry.TryGet(model, out var radio))
            throw new CliError($"Unsupported radio '{model}'. Supported: {RadioRegistry.SupportedList()}.", 1);
        return radio;
    }
}
