...
public sealed class D878uvProtocol : IRadioProtocol
{
    ...
    public async Task WriteImageAsync(ISerialLink link, ReadOnlyMemory<byte> image, IProgress<TransferProgress>? progress, CancellationToken ct)
    {
        ...
        foreach (var region in Layout.Regions)
        {
            int offset = Layout.OffsetOf(region.Address);
            for (int n = 0; n < region.Length; n += WriteChunk)
            {
                await WriteChunkAsync(link, region.Address + (uint)n, image.Slice(offset + n, WriteChunk), ct);
                done += WriteChunk;
    ...
    public async Task EndSessionAsync(ISerialLink link, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var ack = new byte[1];
            await TryReadExactAsync(link, ack, CommandTimeout, ct);       // tolerate silence
        }
        catch { /* teardown is best-effort */ }
        finally { _inProgramMode = false; }
    }
